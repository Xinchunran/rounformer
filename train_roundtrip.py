...
class RoundtripModel(object):
    ...
    def discriminators_loss(self, x, y):
        ...
        fake_y = self.g_net(x)
        fake_x = self.h_net(y)
        fake_x = fake_x.transpose(0,1)
        fake_y = fake_y.transpose(0,1)
        dx = self.dx_net(x)
        dy = self.dy_net(y)
...
if __name__ == "__main__":
    ...
    g_net = model.Generator(latent_dim = x_dim, out_shape= y_dim)   
    h_net = model.Generator(latent_dim= y_dim, out_shape = x_dim)
    dx_net = model.Discriminator(inp_shape = x_dim)
    dy_net = model.Discriminator(inp_shape = y_dim)
